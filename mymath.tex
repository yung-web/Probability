...
%include additional size parameter
\newcommand{\mysizebox}[2]{
\begin{tcolorbox}[width=#1\textwidth, colframe=red]
#2
\end{tcolorbox}
}

% text in aboluste position based on texpos pacakge
% 1. box size, 2. 3.: position, 4: contents
\newcommand\myabstext[1]{%
  \begin{textblock*}{\textwidth}(0pt,0.15\textheight)
    \raggedleft
    #1
  \end{textblock*}
}
...
\newcommand{\pth}{\ensuremath{p_{\Theta}(\theta)}}
...
\newcommand{\pthcx}{\ensuremath{p_{\Theta|X}(\theta|x)}}
\newcommand{\pxcth}{\ensuremath{p_{X|\Theta}(x|\theta)}}
%\newcommand{\fthcx}{\ensuremath{f_{\Theta|X}(\theta|x)}}
%newcommand{\fxcth}{\ensuremath{f_{X|\Theta}(x|\theta)}}
