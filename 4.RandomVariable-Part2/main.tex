...
\item<2-> Models a system evolution over time: Continuous time vs. Discrete time.
\bci
\item In many cases, continuous case is the some type of \redblank{3}{limit} of its corresponding discrete case.
\eci

\item<4-> Can we mathematically describe how geometric and exponential rvs meet each other in the limit?

\item<5-> \magenf{Key idea}
\bci
\item Continuous system = Discrete system with \bluef{infinitely many slots whose duration is infinitely small}.


\item $n$-th system $\xrightarrow{n \rightarrow \infty}$ limiting system
\eci

\mycolorbox{
$X^n_{geo}(p_n)$  with CDF $F^n_{geo}(\cdot)$ $\xrightarrow{n \rightarrow \infty}$
$X_{exp}(\lambda)$ with CDF $F_{exp}(\cdot)$
}
