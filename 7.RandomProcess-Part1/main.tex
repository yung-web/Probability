...
\item<1-> What we want to survey: How available are two buses in a city?
...
\item<5-> (i) M1 $=$ M2? (ii) M1 $>$ M2? (iii) M1 $<$ M2?    
...
\item We know: in $\pp{\lambda}$, inter-arrival time $\sim \text{Exp}(\lambda)$ 
...
\item<6-> Why not $\text{Exp}(\lambda)$? \onslide<7->{An observer who arrives at an
