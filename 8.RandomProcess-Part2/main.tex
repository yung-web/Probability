...
\item<2-> A special type of Markov chain where the states are \redf{linearly arranged}
  and transitions can occur only to a \redf{neighboring} state. 
...
\item<3-> \bluef{Every state} will be visited an \bluef{infinite} number of times

\item<4->[\redf{(Q)}] \magenf{Steady-state} behavior: what are the
  \redf{long-term average frequencies} of states?
...
\item<7-> \bluef{Transient}
  states will be visited a \bluef{finite} number of times. Then, the MC
...
\item<8->[\redf{(Q)}] \magenf{Transient} behavior: what is the \redf{first recurrent state to be entered} as
  well as the \redf{time until this happens}?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Special Case: Every Recurrent State is Absorbing}
...
\item Rather than dealing with a general MC, let's focus on the Markov
  chain that \bluef{every recurrent state is absorbing}.


\item<2-> \defi A state $k$ is \bluef{absorbing,} if
...
\vspace{-0.2cm}
\aleq{
  a_1 = \onslide<5->{0}, \quad a_6 =\onslide<6->{1}, \quad a_2 = \onslide<7->{0.2a_1 + 0.3a_2 + 0.4a_3 + 0.1 a_6},
\quad a_3 = \onslide<8->{0.2 a_2 + 0.8 a_6}
}
\eci

\mysmalltwocols{0.25}
{\vspace{-0.5cm}
\mypic{0.9}{L9_absorption_ex1.png}
}
{
\bci
...
\item<2-> \redf{(Q)} Probability that the state eventually enters the
  recurrent class $\{4,5 \}$?
...
{
\vspace{-0.4cm}
\onslide<1->{\mypic{0.9}{L9_absorption_ex2.png}}
\vspace{-0.6cm}
\onslide<4->{\mypic{0.85}{L9_absorption_ex1.png}}
}
...
\item[\redf{(Q)}] Starting from a transient state $i,$ what is the
  \bluef{expected number of steps until a recurrent state is entered} (which
  we call \redf{absorption})?
...
\aleq{
\mu_1 &= \mu_4 = \onslide<4->{0} \quad \text{(for \orangef{recurrent} states)}\cr
\mu_2 &= \onslide<5->{1 +  0.4\mu_2 + 0.3 \mu_3}, \quad \mu_3 =
\onslide<6->{1 + 0.3\mu_2 + 0.4\mu_3}
\quad \text{(for \orangef{transient} states)}
}
...
\vspace{-1.2cm}
\onslide<1->{\mypic{0.4}{L9_spiderfly_onlyfigure.png}}
...
\myvartwocols{0.25}{0.7}{0.29}
{
\small
\plitemsep 0.1in
\bci
\item<2->[\redf{(Q)}] \bluef{Mean first passage time.} Starting from $i,$ expected number of transitions $t_i$ to reach $s$ for the first time?

\item<4->[\redf{(Q)}] \bluef{Mean first recurrence time.} Starting from $s,$ expected number of transitions $t_s^\star$ to reach $s$ for the first time?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Review Questions (1)}
