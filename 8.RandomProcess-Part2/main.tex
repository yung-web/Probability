...
\redf{r_{ij}(n)} \eqdef \cprob{X_n = j \mid X_0 =i}
$
...
\small
\aleq{
\onslide<3->{&\magenf{r_{ij}(n)} = \cprob{X_n = j \mid X_0 =i} =} \cr
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{More Generalized Chapman-Kolmogorov Equation}
...
\item<8->  Then, $\mat{P}^{(2)}= \mat{P} \times \mat{P} = \mat{P}^2.$
  Further, by induction, \redf{$\mat{P}^{(n)} \eqdef \mat{P}^n$}
...
\begin{frame}{Periodicity}
  ...
  \centering
  \includegraphics[width=0.25\columnwidth]{L9_periodic_1.png}
  \hspace{1cm}
