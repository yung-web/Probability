...
\item $n$-step Transition Probability and Classification of States
...
\item<2-> \defi Let $X_1, \ldots, X_n, \ldots$ be a sequence of random
  ...
  \set{S},$ $n\geq 0,$ the following \blueblank{4}{Markov property} is
  satisfied:\\
  for all $n \geq 0,$ all $i,j \in \set{S},$ and all possible
  sequences $i_0, \ldots, i_{n-1}$ of earlier states, 
\mycolorbox{
\centering
  \redblank{4}{$\cprob{X_{n+1} = j \vert X_{n} = i}$}  =
  $\cprob{X_{n+1} = j
      \vert X_n = i, X_{n-1}=i_{n-1}, \ldots, X_0=i_0}$
}

\item<5->  \magenf{Alternate definition via conditional independence.}
...
\item<1-> The value that $X_n$ can take is called \redf{\em
    state}. Thus, the space $\set{S}=\{1, \ldots, m \}$ is called \redf{\em state space}.

\item<2-> We will focus on the MC of \redf{time homogeneity.} The probability $\cprob{X_{n+1} = j \vert X_{n} = i}$ does NOT depends on $n.$ 

  \bci
  \item<3-> In the machine failure example, $\cprob{X_{n+1}=1 | X_n =1} =
    1-b$ $\implies,$ for example, $\cprob{X_{100}=1 | X_{99} =1}
...
$\sum_{j=}^m p_{ij} =1$ (for each row $i$, the column sum = 1)
\eci 
\end{frame}
...
\item<2-> \redf{State Transition Diagram}

\mypic{0.35}{L9_machine_std.png}

  
\item<3-> Transition probability matrix and state transition diagram
  are the complete descriptions of a given Markov chain.
...
\bce 
\item Any convient way of (visually) describning a MC?
