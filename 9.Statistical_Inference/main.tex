...
\bci
\item \redf{Estimation}: \onslide<2->{$\theta$: prob. of head?}
\item Hypothesis testing: \onslide<2->{$\theta = 1/2$ or $\theta = 1/4$?}
\item Significance testing: \onslide<2->{$\theta = 1/2$ or not?}
\eci
...
\aleq{
\hat{\theta}_{\map} &= \arg \max_{\theta} p_{\Theta | X} (\theta | x)= \arg \max_{\theta} \frac{p_{X| \Theta}(x|\theta) p_\Theta(\theta)}{\px} = \frac{1}{\px}\arg \max_{\theta} \orangef{p_{X| \Theta}(x|\theta)} \redf{p_\Theta(\theta)}
}

\item<3-> ML in the classical inference
\aleq{
\hat{\theta}_{\ml} = \arg \max_{\theta} \orangef{p_X(x;\theta)}
}
...
\hfill \lecturemark{Slides 17, 35, 45, and 56 for more details}
