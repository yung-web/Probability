...
\myblock{Convergence in Distribution: $Y_n \xrightarrow{\text{in distribution}} Y$}
{
\onslide<2->{
For every $y,$
$$
...
M_X(s) = \expect{e^{sX}}
$$
- If clear, we omit $X$ and use just $M(s).$
\aleq{
\onslide<3->{
...
\item<5->[Ex2)] $X \sim \set{N}(0,1)$ (homework problem)
\aleq{
M(s) = e^{s^2/2}
}
...
\item<3->  Show: MGF of $Z_n$ converges to MFG of $\set{N}(0,1)$ (using inversion property)
\eci
...
\item<1-> What we want: \bluef{a sequence of rvs $(Y_n)_{n=1, 2, \ldots}$ converges to a rv $Y$} in some sense, different from \magenf{convergence in probability} and \magenf{convergence in distribution} which we discussed earlier.

\item<2-> \orangef{Almost Sure Convergence}: $Y_n \xrightarrow{\text{a.s.}} Y$
\mycolorbox{
\centering
$\bprob{\big \{\omega \in \Omega| \lim_{n \rightarrow \infty} Y_n(\omega) = Y(\omega) \big\}}$
}
