...
\begin{frame}{Bayes' Rule: 4 Versions}
...
\bluef{ 
    \aleq{
    X_1 &= \theta + W_1, \quad W_1 \sim \set{N}(x_1,\sigma_1^2)\cr
       \vdots & \cr
      X_n &= \theta + W_n, \quad  W_n \sim \set{N}(x_n,\sigma_n^2)
    }
}    
\vspace{-0.6cm}
...
\mycolorbox{
  \thm The product of two Gaussians $\set{N}(\mu_0,\nu_0)$ and
...
\mycolorbox{
  \thm The product of $n+1$ Gaussians $\set{N}(\mu_0,\nu_0),$
...
\begin{frame}{Example: Parameter Inference with Normal Prior (2)}
  ...
  \item $n$ observations of $\theta$: $W_i \sim
    ...
    \mycolorbox{
\centering
      $X_i = \theta + W_i, \quad W_i \sim \set{N}(x_i,\sigma_i^2),$ 
    $\quad i=1, \ldots, n$
    }
...
\hfill \lecturemark{Slide 17 for more details}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example 4: Parameter Inference with Normal Prior}
