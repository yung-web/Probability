...
\item<3-> \redf{Question:} How should I change my belief, given some additional information?

\item<4-> Need to build up a new theory, which we call \redblank{5}{conditional probability}.
...
\bci
  ...
  \item $\cprob{B | B}=1$ from our common sense. True?
\eci
}
...
\mycolorbox{
$$
\cprob{A \mid B} \eqdef \frac{\cprob{A \cap B}}{\cprob{B}}, \quad \text{for} \quad \cprob{B} >0.
$$
}
...
\item<2-> \orangef{Partition} of $\Omega$ into $A_1,A_2,A_3$
...
\item \onslide<6->{Using $\cprob{ A\cap B | C} = \frac{\prob{B \cap (A \cap C)}}{\cprob{C}} = \frac{\cprob{B | A\cap C} \cprob{A \cap C}}{\cprob{C}} = \cprob{A | C} \cprob{B | C},$ }
...
Suppose that $A$ and $B$ are independent. If you heard that $C$ occurred,  $A$ and $B$ are still independent? \hfill \lecturemark{VIDEO PAUSE}
