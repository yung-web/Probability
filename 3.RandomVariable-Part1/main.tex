...
\medskip
\onslide<6->{\msg When some rv $X$ is write as a linear combination of other rvs, it is often easy to handle $X.$}
}
\end{frame}
...
\item Using the definition of expectation,
\aleq{
...
\item<3-> Let's use TET, for which consider
...
\aleq{
\cprob{X = k}& = (1-p)^{k-1} p, \quad \greenf{\cprob{X > k}} = 1 - \sum_{k'=1}^k (1-p)^{k'-1} p = \greenf{(1-p)^k}
}
...
\item<6-> Total expectation theorem and memorylessness helps a lot. 
\eci
}
{
\plitemsep 0.1in
